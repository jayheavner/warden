...
LAB="$(cd "$LAB" && pwd -P)"
REPO="$LAB/alpha"; WT="$REPO/.claude/worktrees/w1"
...
P=0; F=0
run() { sandbox-exec -f "$PROFILE" /bin/sh -c "$1" >/dev/null 2>&1; }
...
ok "cd '$WT' && git add -A && git commit -qm w" "worktree commit allowed (shared .git writes)"
